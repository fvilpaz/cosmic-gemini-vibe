...
if (content.match(regexToMatch)) {
   content = content.replace(regexToMatch, cyberBlock + "  if (level.environment === \"mad-fields\") {\n    return (\n      <AnimatePresence");
   fs.writeFileSync('src/components/LevelBackground.tsx', content);
   console.log('Successfully injected cyberpunk block before mad-fields render block!');
} else {
   console.log('Regex failed to find mad-fields block');
}
