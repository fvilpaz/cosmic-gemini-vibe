...
export default function App() {
  ...
  const [ufoProjectiles, setUfoProjectiles] = useState<
    { id: string; x: number; y: number }[]
  >([]);
  const ufoProjectilesRef = useRef<any[]>([]);
  ...
  const startLevel = useCallback((idx: number) => {
    ...
    const freshEnemies = level.enemySpawns.map((spawn, i) => {
      ...
      const isRandomCow =
        ...
        level.number !== 7 &&
        level.number !== 6 &&
        level.number !== 5 && level.number !== 10;
                        ...
                        {isShipOnFire && (
                          <motion.div
                            ...
                            style={{
                              borderRadius: "6px 40px 40px 6px",
                              background:
                                "radial-gradient(ellipse at 60% 50%, rgba(251,191,36,0.95) 0%, rgba(239,68,68,0.8) 45%, transparent 80%)",
                            }}
                            animate={{
                              opacity: [0.6, 1, 0.7, 1],
                              scale: [1, 1.15, 1, 1.2],
